...
flags.DEFINE_string(
    "test_file", None,
    "the single test file need to test."
)
...
class DataProcessor(object):
    ...
    @classmethod
    def _read_data_from_string(cls, input_string):
        lines = []
        words = []
        labels = []
        # print(f.read())
        word_list = []
        strings = input_string.replace('\r','').split('\n')
        for string in strings:
            for i in range(len(string)):
                word_list.append(string[i] + " O")
            word_list.append("")
        print(word_list)
...
class NerProcessor(DataProcessor):
    ...
    def get_test_examples(self,data_dir='',input_string = '',single_file = False,input_str =False):
        if single_file == True:
            return self._create_example(
                self._read_data(data_dir), "test")
        elif input_str == True:
            return self._create_example(
                self._read_data_from_string(input_string),"test"
            )
        else:
            return self._create_example(
    ...
    def get_labels(self):
        ...
        return ["O", "B-Time", "I-Time", "B-Organization", "I-Organization", "B-Location", "I-Location", "X", "[CLS]", "[SEP]"]
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        if FLAGS.test_file == None:
            predict_examples = processor.get_test_examples(FLAGS.data_dir)
        else:
            predict_examples = processor.get_test_examples(FLAGS.test_file,single_file=True)
